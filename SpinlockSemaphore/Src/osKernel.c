...
uint32_t period_tick;  /*Used for clock tick of periodic thread*/
...
/* Each thread of stacksize of 100 i.e. 400bytes */
...
{
	MILLIS_PRESCALER = (BUS_FREQ/1000);  /*Convert seconds to milliseconds - divide 16MHz clock by 1000*/
}
...
void osSchedulerLaunch(void)
{
	...
	/* Skip LR and PSR do this by adding 4 to SP */
	__asm("ADD SP,SP,#4");
...
void osSchedulerRoundRobin(void)
{
	/* PERIOD defined in the header file
	 * if same as period_tick, then execute periodic task
	 */
	if((++period_tick) == PERIOD)
	{
		/* Execute periodic task, then task switcher selects the next task */
		(*task3)();
...
void tim2_1hz_interrupt_init(void)
{
	...
	/* Set timer prescaler to slow down the clock */
	TIM2->PSC = 1600 - 1;  /*Default clock 16MHz=> 16000000/1600=10000*/
